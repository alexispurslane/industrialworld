...
--- tile it collides with (per the existing `should_collide`, i.e. the
--- Solid mask). On the current map only Walls are Solid, so an entity
--- falls through Floor/Open until a Wall is below (or it hits z=0, the
--- floor of the world).
---
...
--- horizontal step, and overrides `init` to settle on spawn ("on start").
--- Mix it BEFORE Drawable so its `move` wins first-wins over Drawable's
--- plain Position.move.
---
...
--- Initialize position + collision mask (Collidable leaf), set the z
--- layer, store the gravity flag, and settle onto the ground ("on start").
---@param x number
---@param y number
---@param z integer
...
--- blocks it. Otherwise decrements z and re-checks.
...
--- Collision-guarded step PLUS re-settle. Delegates the horizontal step
--- to Collidable (which already checks the destination tile's mask and
--- bounds, and returns false if blocked). On a successful step, re-run
--- gravity: the new x,y column may have open space below. Only emits a
--- move-effect (via the caller's "moved") when the step actually happened.
---@param dx number
---@param dy number
---@return boolean moved  true if the step was taken, false if blocked.
function PhysicsObject:move(dx, dy)
    if not Collidable.move(self, dx, dy) then
        return false
    end
    self:fall()
    return true
